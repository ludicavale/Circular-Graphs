...
function drawNodes(draw,node,size,symbol) {
	...
	if (symbol==ROUND){
		...
		drawArcs(draw,child,size);
		draw.fillStyle = 'rgba(0, 0, 0, 0.9)';
	}else{
		draw.fillStyle = 'rgba(0, 0, 0, 1.0)';
...
function drawArcs(draw,node,size) {
	var n = node.children.length;
	draw.arc(node.x,node.y, n*size, (Math.PI/180)*0, (Math.PI/180)*360, false);
	draw.lineWidth = 1;
	draw.fillStyle = 'rgba(0, 0, 0, 0.9)';
}
